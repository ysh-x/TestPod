...
public class GenerateSI {
	...
	List<List<Float>> SIM = new ArrayList<>();
	List<Float> SI = new ArrayList<>();
	List<Float> CI = new ArrayList<>();
	
	int numberOfQuestions = 0;

	List<Pair<Integer, Integer>> limitersS = new ArrayList<>();
	List<Pair<String, String>> finalStructureS = new ArrayList<>();
	List<Pair<Integer, Integer>> answerCountS = new ArrayList<>();

	List<Pair<Integer, Integer>> limitersF = new ArrayList<>();
	List<Pair<String, String>> finalStructureF = new ArrayList<>();
	List<Pair<Integer, Integer>> answerCountF = new ArrayList<>();
	...
	public ReportBean generate(int testID, int examID) throws IOException {
		...
		for (int i = 0; i < numberOfQuestions; i++) {
			...
			List<Float> delim = new ArrayList<>();
			delim.add((float) -1);
			SIM.add(delim);
		}
